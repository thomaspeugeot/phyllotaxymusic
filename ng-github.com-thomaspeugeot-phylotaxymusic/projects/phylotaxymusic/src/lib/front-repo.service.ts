			...
			(observer) => {
				combineLatest(
					...
					([
						...
						// init the arrays
						this.frontRepo.array_Diagrams = []
						this.frontRepo.map_ID_Diagram.clear()

						diagrams.forEach(
							diagramAPI => {
								let diagram = new Diagram
								this.frontRepo.array_Diagrams.push(diagram)
								this.frontRepo.map_ID_Diagram.set(diagramAPI.ID, diagram)
							}
						)
	...
	public connectToWebSocket(GONG__StackPath: string): Observable<FrontRepo> {
		...
		return new Observable(observer => {
			this.socket!.onmessage = event => {
				...
				backRepoData.DiagramAPIs.forEach(
					diagramAPI => {
						let diagram = new Diagram
						this.frontRepo.array_Diagrams.push(diagram)
						this.frontRepo.map_ID_Diagram.set(diagramAPI.ID, diagram)
