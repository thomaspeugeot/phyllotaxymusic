...
func (backRepoSpiralLineGrid *BackRepoSpiralLineGridStruct) CommitPhaseOne(stage *models.Stage) (Error error) {
	...
	sort.Slice(spirallinegrids, func(i, j int) bool {
		return stage.SpiralLineGridMap_Staged_Order[spirallinegrids[i]] < stage.SpiralLineGridMap_Staged_Order[spirallinegrids[j]]
	})
...
func (spirallinegridDB *SpiralLineGridDB) DecodePointers(backRepo *BackRepoStruct, spirallinegrid *models.SpiralLineGrid) {
	...
	// ShapeCategory field	
	{
		...
		if id != 0 {
			...
			if !ok {
				log.Println("DecodePointers: spirallinegrid.ShapeCategory, unknown pointer id", id)
