...
func (controller *Controller) GetBezier(c *gin.Context) {
	...
	if err := db.First(&bezierDB, c.Param("id")).Error; err != nil {
		var returnError GenericError
		returnError.Body.Code = http.StatusBadRequest
		returnError.Body.Message = err.Error()
		log.Println(err.Error())
...
func (controller *Controller) UpdateBezier(c *gin.Context) {
	...
	// fetch the bezier
	query := db.First(&bezierDB, c.Param("id"))
	...
	query = db.Model(&bezierDB).Updates(bezierDB)
	if query.Error != nil {
		var returnError GenericError
		returnError.Body.Code = http.StatusBadRequest
		returnError.Body.Message = query.Error.Error()
