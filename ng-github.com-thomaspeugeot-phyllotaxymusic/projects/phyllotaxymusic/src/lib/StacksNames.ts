// generated from ng_file_enum.ts.go
export enum StacksNames {
	// insertion point	
	Phylotaxy = "phyllotaxymusic",
	GongsvgStackName = "gongsvg",
	SidebarTree = "sidebar tree",
	ToneStackName = "gongtone",
	CursorStackName = "cursor",
	LoadStackName = "load",
	ButtonStackName = "button",
	RootSplitStackName = "",
	GongLibSliderStackName = "slider",
	GongtreeStackName = "gongtree",
	GongtableStackName = "gongtable",
	GongsimStackName = "gongsim",
}

export interface StacksNamesSelect {
	value: string;
	viewValue: string;
}

export const StacksNamesList: StacksNamesSelect[] = [ // insertion point	
	{ value: StacksNames.Phylotaxy, viewValue: "phyllotaxymusic" },
	{ value: StacksNames.GongsvgStackName, viewValue: "gongsvg" },
	{ value: StacksNames.SidebarTree, viewValue: "sidebar tree" },
	{ value: StacksNames.ToneStackName, viewValue: "gongtone" },
	{ value: StacksNames.CursorStackName, viewValue: "cursor" },
	{ value: StacksNames.LoadStackName, viewValue: "load" },
	{ value: StacksNames.ButtonStackName, viewValue: "button" },
	{ value: StacksNames.RootSplitStackName, viewValue: "" },
	{ value: StacksNames.GongLibSliderStackName, viewValue: "slider" },
	{ value: StacksNames.GongtreeStackName, viewValue: "gongtree" },
	{ value: StacksNames.GongtableStackName, viewValue: "gongtable" },
	{ value: StacksNames.GongsimStackName, viewValue: "gongsim" },
];
